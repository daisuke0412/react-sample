...
*   **更新処理の複雑さ**: データ更新（Mutation）時の「通信中」「成功」「失敗」といった状態管理や、成功時と失敗時の分岐処理などが複雑になりがちです。
...
sequenceDiagram
  ...
  participant TQ as useQuery
  participant API as バックエンドAPI

  Component->>TQ: queryKeyの設定/更新
  TQ->>TQ: クエリキーでキャッシュ検索
  alt クエリキーに紐づくキャッシュあり
    TQ->>TQ: fresh(有効)か、stale(無効)か判定
    alt キャッシュがfresh状態の場合
      TQ-->>Component: キャッシュを返却
    else キャッシュがstale状態の場合
      TQ-->>Component: キャッシュを返却
...
export function useItemCreate() {
  ...
  const mutation = useMutation({
    ...
    mutationFn: (newItem: CreateItemParams) => api.post<Item>("/items", newItem),
    // 成功時の処理
    onSuccess: () => {
...
1.  **直感的な処理フロー**: 更新処理の前後や、成功時・失敗時の処理をオプション（`onSuccess`, `onError`）で宣言的に記述でき、複雑な `if` 文による分岐を減らせます。
2.  **UI実装との親和性**: `isPending`（実行中）や `error`（エラー情報）などの状態変数が提供されるため、「送信中のローディング表示」や「エラーメッセージ」の実装が非常に簡単です。
3.  **データ再取得との連携**: 更新成功時に `invalidateQueries` を呼ぶことで、「キャッシュを無効化して最新データを再取得する」という連携がスムーズに行えます。
