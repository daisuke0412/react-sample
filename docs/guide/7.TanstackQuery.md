...
また、TanStack Query は、キャッシュが「新しい (fresh)」か「古い (stale)」かを常に管理しています。  
キャッシュは下記のような場合に「古い (stale)」とみなされます。

*   **データ取得から staleTime が経過した場合**: `staleTime`（鮮度保持期間）を過ぎると、そのデータは古いと判断されます。
...
| **Fresh状態** | 検索ワードを消した状態で検索し、**すぐに**再度「イヤホン」で検索 | **一瞬で表示** | データ取得から10秒以内のため、キャッシュ（Fresh）が使われる。API通信は発生しない。 |
