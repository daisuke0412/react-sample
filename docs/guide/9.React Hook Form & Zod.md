...
*   **name**: 必須項目（1文字以上）
*   **price**: 必須項目、1以上の整数
*   **description**: 必須項目（1文字以上）
*   **status**: "on_sale" または "sold_out" のいずれか
...
*   `resolver: zodResolver(itemSchema)`: ZodスキーマをReact Hook Formに接続し、バリデーションを自動化します。
*   `{...register("name")}`: 入力値の取得、変更検知、バリデーション実行などをコンポーネントに紐付けます。
*   `{...register("price", { valueAsNumber: true })}`: 入力値を数値として取得します。これにより `z.number()` のバリデーションが正しく機能します。
*   `error={!!errors.name}`: エラーがある場合に赤枠を表示します。
