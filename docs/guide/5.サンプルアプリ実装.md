...
まず、型定義を `app/features/items/types.ts` に作成しておきます（ディレクトリがない場合は作成してください）。
...
先ほど `app/features/items/types.ts` に作成した型定義を使用します。
...
React Router v7 の `clientLoader` を使用して、ページ遷移前にデータを取得するロジックを定義します。
`app/features/items/hooks/useItemsRoute.ts` に記述します。
...
**app/features/items/components/ItemCard.tsx**
```tsx
// MUIのCardコンポーネントを使用してリッチな表示にします
export function ItemCard({ item, handleClick }: ItemCardProps) {
  // ... (実装詳細はソースコードを参照)
}
```

※ `ItemList` や `ItemHeader` のコードは長くなるため省略します。実際のソースコードを確認してください。

### 5.2.4 ページコンポーネントとカスタムフック

ロジックと表示を分離するため、カスタムフック `useItems` を作成し、ページコンポーネント `ItemsPage` から利用します。
...
最後に、`app/routes/items._index.tsx` でルートとコンポーネント、Loaderを紐付けます。
...
これで `/items` にアクセスすると、モックデータがロードされ、商品一覧が表示されます。
...
MUI を使用してフォーム画面を作成します。React Router の `Form` コンポーネントを使用することで、JavaScript が有効な環境では `clientAction` が、無効な環境でも標準の POST 送信が機能するようになります（今回は SPA なので `clientAction` が動作します）。
...
`app/routes/items.new._index.tsx` でルート定義を行います。
`clientAction` をエクスポートすることで、フォーム送信時の処理を React Router に委譲します。
...
sequenceDiagram
    ...
    Hook-->>Page: { isSubmitting: false }
    Page-->>user: 商品作成フォーム表示

    Note over user, Hook: 2. フォーム送信フェーズ
    user->>Page: 入力 & 送信
    Page->>ReactRouter: Form送信 (POST)
    ReactRouter->>Action: clientAction実行
    
    alt バリデーションエラー
        Action-->>ReactRouter: エラー情報 (ActionData)
        ReactRouter-->>Route: ActionData設定
        Route->>Page: 再レンダリング
        Page->>Hook: useItemCreate()
        Hook->>Route: useActionData()
        Route-->>Hook: { errors }
        Hook-->>Page: { fieldErrors }
...
### 5.4.1 データ取得・更新ロジック (clientLoader / clientAction)
...
`app/routes/items.$id.tsx` で動的ルートパラメータ（`$id`）を持つルートを定義します。
...
sequenceDiagram
    ...
    participant Mock as バックエンド<br>（モックAPI）
    ...
    alt バリデーションエラー
        ...
        Note over ReactRouter, Hook: 3. 再検証フェーズ (Revalidation)
...
React Router v7 では、ルートコンポーネントで例外（Error）がスローされると、自動的に最も近い `ErrorBoundary` がレンダリングされます。
これにより、予期せぬエラーが発生した場合でも、アプリケーション全体がクラッシュすることなく、ユーザーフレンドリーなエラー画面を表示できます。

もし `ErrorBoundary` を設定していない場合、開発モードでは以下のようなスタックトレースが表示され、ユーザーにとって非常に分かりにくい（かつ不安を与える）画面になってしまいます。

![Stacktrace Error](../picture/stacktrace_error.png)

本節では、このような画面を回避し、適切なエラーメッセージを表示する仕組みを実装します。
