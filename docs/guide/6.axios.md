...
本章では、Axios を導入し、共通的な API クライアントやインターセプターといった強力な機能を `api-client` に集約して実装しました。
その結果、各機能（カスタムフックなど）からは非常にシンプルなコードで、これらの共通機能を利用した API 通信が行えるようになりました。

Axios を活用することで、**シンプルなコード**で**変更に強い** API 通信処理を実装できるようになりました。
