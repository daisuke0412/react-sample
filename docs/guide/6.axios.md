...
アプリと同様に ES Modules (`import` / `export`) を扱うため、`"type": "module"` を追加します。
また、`node --watch` を使ってファイルの変更を検知して自動再起動するようにし、`ts-node/register` を使って TypeScript ファイルを直接実行できるようにします。
