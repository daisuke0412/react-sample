...
他のライブラリ（Redux, Context API）と比較して、主に以下の3つのメリットがあります。

1.  **コードがシンプル**  
    Redux のような複雑なボイラープレート（Action, Reducer, Dispatch...）が不要で、非常に少ないコード量で状態管理を実装できます。

2.  **再レンダリングの制御が容易**  
    Zustand は「自分が使っているデータが変化した時だけ再レンダリングする」という制御が自動で行われるため、意識せずとも効率的で最新のストア状態を画面に反映できます。

3.  **コンポーネント外からのアクセス**  
    これが最大の特徴です。`useState` や `useContext` と異なり、**Reactコンポーネントの外側（普通の関数やクラス）からも状態にアクセス・更新ができます**。
    これにより、「API通信を行うユーティリティ関数」から直接「ローディング状態」を操作するといった連携が容易になります。
...
ここでは、TanStack Query の `useMutation` でAPIを呼び出し、その成功時（`onSuccess`）に Zustand ストアの更新関数（`setUser`）を実行するパターンを確認します。
...
export function useAuth(onClose?: () => void) {
  ...
  // ... (ログアウト処理やフォームハンドリングなど)
  
  return {
    onLogin: handleLogin, // フォーム送信ハンドラ
    // ...
  };
}
```
