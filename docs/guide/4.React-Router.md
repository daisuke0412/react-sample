...
このように各画面で個別に実装していくと、
- 担当者によって書き方がバラバラになり、コード理解が難しくなる
...
1.  **「データ取得完了前の状態」を考慮しなくていい**
    *   コンポーネントがレンダリングされる時点で、`商品データ取得処理`は完了しています。
2.  **データ取得のロジックがコンポーネントから消える**
    *   「データ取得方法」は Router に任せ、コンポーネントは「データをどう表示するか」に集中できます。
...
比較してみるとclientLoaderはデータ取得、ItemsPageコンポーネントはそのデータを受け取って表示、というように役割が明確に分かれ、ソースコードもスッキリしていることが分かると思います。
...
`clientAction` を使うと、**コンポーネントは入力フォームを定義するのみで、送信処理を外出し** できます。
...
2.  **各画面の実装がシンプルになる**
    ...
    *   各画面で個別のエラーハンドリングをしない場合は、エラー対応は ErrorBoundary に任せることができます。
...
> 特にAPI通信周りは、404や500エラーなどの処理をすべての画面に書くのは大変ですが、ErrorBoundaryで共通的なエラーハンドリングを行うことができるのも嬉しいポイントです。  
...
Framework モードの最大のメリットは、**「コンポーネント（UI）」と「データ取得や処理」を明確に分離できること**です。

エラー表示や、今回は紹介しきれなかった HTML の meta データ設定（タイトルやディスクリプション）などの共通的な処理も、React コンポーネントの中に無理やり実装するのではなく、React Router の機能としてきれいに切り出すことができます。

これにより、
*   **コードがシンプルになり、どこで何をやっているのかがすぐに理解できる**
*   **変更や機能追加が容易になる**
