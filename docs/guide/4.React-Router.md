...
- **データ取得ロジックをコンポーネントから分離させ、シンプルなコード構成にできる**
...
- **データ取得や送信などのロジックをコンポーネントから分離させ、シンプルなコード構成にできる**
...
1.  **状態管理（State）が大量に必要になる**
    *   入力値、エラーメッセージ、成功フラグなどをすべて `useState` で管理する必要があります。
...
- **送信ロジックをコンポーネントから分離させ、シンプルなコード構成にできる**
